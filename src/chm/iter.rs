use crate::chm::{ConHashMap, HashMap, util};
use std::hash::{BuildHasher, Hash};
use std::collections::hash_map;
use std::mem;
use crate::chm::util::SharedValue;
use std::sync::Arc;
use crate::chm::lock::{RwLockReadGuard, RwLockWriteGuard};
use crate::chm::t::Map;
use crate::chm::mapref::multiple::{RefMulti, RefMutMulti};
...
impl<K: Eq + Hash, V, S: BuildHasher + Clone> OwningIter<K, V, S> {
    pub fn new(map: ConHashMap<K, V, S>) -> Self {
        Self {
            map,
            shard_i: 0,
            current: None,
